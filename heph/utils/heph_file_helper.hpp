...
HephResult
heph_get_file_size_fd(const int fd, size_t *const size)
{
        if (!size || fd < 0)
        {
...
HephResult
heph_enumerate_directory_entries(const std::string &dir_path, std::vector<std::string> &entries)
{
        DIR *dir;
        struct dirent *ent;
...
HephResult
heph_get_file_size_path(const std::string &path, size_t *const size)
{
        if (!size)
        {
...
// i dont care
inline bool
heph_file_exists(const std::string &path)
{
        return !(bool)access(path.c_str(), F_OK);
}

HephResult
heph_read_file_to(void *const dest, size_t *const size, const std::string &path)
{
        int fd = open(path.c_str(), O_RDONLY);
        if (fd == -1)
        {
