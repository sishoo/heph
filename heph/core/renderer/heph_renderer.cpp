...
{
        r->instance_thread_pool = application_thread_pool;
...
/* Renders a single frame, call in loop */
...
{       
        ...
        const uint32_t frustum_size = sizeof(float) * 32;
        uint32_t compute_shader_push_constant_data[frustum_size + sizeof(uint32_t)] = {r->object_buffer_swap};
        memcpy(push_data + 1, r->camera->frustum_vertices, frustum_size);
        vkCmdPushConstants(
                frame_command_buffer, 
                r->compute_pipeline_layout, 
                VK_SHADER_STAGE_COMPUTE_BIT, 
                0, 
                frustum_size + object_buffer_swap_size, 
                (void *)compute_shader_push_constant_data
        );
