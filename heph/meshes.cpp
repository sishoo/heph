...
Meshes::Meshes(VkDeviceSize prealloc_sb) : vertex_tsb(0), index_tsb(0), paths(prealloc_sb) {}

Meshes::Meshes(const std::string &path) : vertex_tsb(0), index_tsb(0) { queue_hmodl(path); }

Meshes::Meshes(const std::vector<const std::string &> &batch_paths) : vertex_tsb(0), index_tsb(0) { paths = batch_paths; }

inline Meshes
Meshes::with_capacity(VkDeviceSize prealloc_sb) { return Meshes(prealloc_sb); }

inline Meshes
Meshes::from_path(const std::string &path) { return Meshes(path); }

inline Meshes
Meshes::from_batch(const std::vector<const std::string &> &batch_paths) { return Meshes(batch_paths); }

Meshes
Meshes::from_dir(const std::string &path) { 
        Meshes m = Meshes();
        m.queue_hmodl_directory(path);
        return m; 
}
...
Meshes::queue_hmodl(const std::string &path)
{
	...
	if (heph_get_hmodl_xattr(path) == HephResult::Failure)
	{
