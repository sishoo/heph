...
Meshes::Meshes(size_t prealloc_sb) : num(0), vertex_sb(0), index_sb(0), paths(prealloc_sb) {}

Meshes::Meshes(const std::string &path) : num(0), vertex_sb(0), index_sb(0), paths(path) {}

Meshes::Meshes(const std::vector<const std::string &> &batch_paths) : num(0), vertex_sb(0), index_sb(0) 
{
    paths = batch_paths;
}

// static inline Meshes
...
HephResult
Meshes::write(void *ptr) const
{
    void *vertex_ptr = ptr;
    void *index_ptr = ptr + vertex_tsb;
    void *normal_ptr = index_ptr + index_tsb;
