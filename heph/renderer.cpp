...
{       
        ...
        r->frame_render_infos = (HephFrameRenderInfo *)malloc(sizeof(HephFrameRenderInfo) * r->swapchain_image_count);
...
void heph_renderer_render_frame(HephRenderer *const r)
{
        ...
        */
        ...
        if (vkResetCommandBuffer(r->command_buffer, 0) != VK_SUCCESS)
        {
                HEPH_ABORT("Failed to reset command buffer.");
        }
	...
	begin_info.flags = VK_COMMAND_BUFFER_USAGE_ONE_TIME_SUBMIT_BIT;
        if (vkBeginCommandBuffer(r->command_buffer, &begin_info) != VK_SUCCESS)
        {
	...
	// image_barrier.newLayout = VK_IMAGE_LAYOUT_GENERAL;
        ...
        translate_image_layout(r->command_buffer, image, VK_IMAGE_LAYOUT_UNDEFINED, VK_IMAGE_LAYOUT_GENERAL);
        ...
        */
        ...
        vkCmdClearColorImage(r->command_buffer, r->swapchain_images[image_index], VK_IMAGE_LAYOUT_GENERAL, &clear_color, 1, &range);
        
        translate_image_layout(r->command_buffer, image, VK_IMAGE_LAYOUT_GENERAL, VK_IMAGE_LAYOUT_PRESENT_SRC_KHR);
