...
void heph_renderer_init_window(HephRenderer *const r, const std::string &name)
{
        ...
        HEPH_COND_ABORT((r->window = glfwCreateWindow(r->window_width, r->window_height, name.c_str(), NULL, NULL)), NULL);
}
...
bool heph_renderer_preprocess_vertex_shader(char *const src, uint32_t src_sb, HephShaderMacroReplacement, uint32_t nmacros)
{
        shaderc_compile_options_t compile_options = shaderc_compile_options_initialize();    

        shaderc_compilation_result_t result = 
                shaderc_compile_into_preprocessed_text(compiler, src, src_sb, shaderc_fragment_shader, "fragment_shader", "main", compile_options);
...
void heph_renderer_preprocess_fragment_shader(shaderc_compiler_t compiler, char *const src, uint32_t src_sb)
{
        shaderc_compile_options_t compile_options = shaderc_compile_options_initialize();    
...
uint32_t *heph_renderer_compile_vertex_shader(shaderc_compiler_t compiler, const std::string &src)
{
...
uint32_t *heph_renderer_compile_fragment_shader(shaderc_compiler_t compiler, char *const src)
{
...
void heph_renderer_init_shader_modules(HephRenderer *const r)
{
        ...
        */
        ...
        /* Vertex shader*/
        HephMappedFile vertex_shader_src = {};
        HephConstString preprocessed_vertex_shader_src = {};
        HEPH_COND_ABORT_NE(heph_file_map_file(&vertex_shader_src, "shader/vertex.vert"), true);
        HEPH_COND_ABORT_NE(heph_renderer_preprocess_vertex_shader(r->shader_compiler, &preprocessed_vertex_shader_src, vertex_shader_src.ptr, vertex_shader_src.sb), true);
        heph_renderer_compile_vertex_shader(r->shader_compiler, preprocessed_vertex_shader_src);
        heph_file_unmap_file(&vertex_shader_src);
        
        /* Fragment shader */
        HephMappedFile fragment_shader_src = {};
        HEPH_COND_ABORT_NE(heph_file_map_file(&fragment_shader_src, "shader/fragment.frag"), true);
        std::string
        heph_renderer_preprocess_fragment_shader(fragment_shader_src_string);
...
{       
        ...
        vkCreateGraphicsPipelines(r->ldevice, );
}
...
{       
        /* This might indicate an overall design flaw. */
        static bool is_first = true;

        const uint32_t resource_index = (r->prev_image + 1) * !(r->prev_image + 1 == r->swapchain_nimages);
        ...
        VkSubmitInfo2 submit_info = {
                ...
                .waitSemaphoreInfoCount = 2 - (uint32_t)is_first,
