...
uint32_t heph_static_meshes_add_hmodl(HephStaticMeshes *const m, heph_string_t *path)
{
        if (m->nmeshes == m->meshes_cap)
        {
                HEPH_COND_ABORT(m->paths = (heph_string_t *)realloc(m->paths, m->meshes_cap * 2), NULL);
        }

        memcpy(&m->paths[m->nmeshes], path, sizeof(heph_string_t));
